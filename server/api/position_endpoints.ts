...
export function registerPositionEndpoints(app: Express) {
  ...
  app.post("/api/pd", async (req: Request, res: Response) => {
    try {
      ...
      const positionDepartment = await storage.createPositionDepartment(
        positionDepartmentData,
      );
  ...
  app.put("/api/pd/:id", async (req: Request, res: Response) => {
    try {
      ...
      // Проверяем, существует ли связь
      const existingPositionDepartment =
        await storage.getPositionDepartment(id);
      ...
      // Обновляем связь
      const positionDepartment = await storage.updatePositionDepartment(
        id,
        updateData,
      );
      res.json({ status: "success", data: positionDepartment });
