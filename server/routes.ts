...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      try {
        ...
        const positionsWithDepts = positions.map((position) => {
          ...
          const linkedDepartments = links.map((link) => {
            ...
            // Находим родительскую должность для этой должности в этом отделе из position_position
            const parentPositionRelation = positionPositions.find(
              (pp) =>
            ...
            if (parentPositionRelation) {
              const parentPosition = positions.find(
                (p) =>
                  p.position_id === parentPositionRelation.parent_position_id,
              );
              if (parentPosition) {
                parentPositionInfo = {
        ...
        res
          ...
          .json({
            status: "error",
            message: "Failed to fetch positions with departments",
          });
      }
    },
  );
  ...
  app.get("/api/positiondepartments", async (req: Request, res: Response) => {
    try {
      ...
      res
        ...
        .json({
          status: "error",
          message: "Failed to fetch position departments",
        });
    }
  });
  ...
  app.get(
    "/api/positionpositions/position/:id",
    async (req: Request, res: Response) => {
      try {
        ...
        const positionPositions =
          await storage.getPositionPositionsByPosition(positionId);
        res.json({ status: "success", data: positionPositions });
      } catch (error) {
        ...
        res
          ...
          .json({
            status: "error",
            message: "Failed to fetch position relationships",
          });
      }
    },
  );
  ...
  app.get(
    "/api/positionpositions/parent/:id",
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(parentPositionId)) {
          return res
            .status(400)
            .json({ status: "error", message: "Invalid parent position ID" });
        }

        const positionPositions =
          await storage.getPositionPositionsByParent(parentPositionId);
        res.json({ status: "success", data: positionPositions });
      } catch (error) {
        ...
        res
          ...
          .json({
            status: "error",
            message: "Failed to fetch subordinate positions",
          });
      }
    },
  );
  ...
  app.get(
    "/api/positionpositions/department/:id",
    async (req: Request, res: Response) => {
      try {
        ...
        res
          ...
          .json({
            status: "error",
            message: "Failed to fetch department position hierarchy",
          });
      }
    },
  );
  ...
  app.post("/api/positionpositions", async (req: Request, res: Response) => {
    try {
      ...
      if (
        !relationData.position_id ||
        !relationData.parent_position_id ||
        !relationData.department_id
      ) {
        return res.status(400).json({
          status: "error",
          message:
            "Position ID, Parent Position ID and Department ID are required",
        });
      }

      // Проверяем, что такой связи еще нет
      const existingRelations = await storage.getPositionPositionsByDepartment(
        relationData.department_id,
      );
      const duplicateRelation = existingRelations.find(
        (rel) =>
          rel.position_id === relationData.position_id &&
      ...
      if (duplicateRelation) {
        return res.status(400).json({
          status: "error",
          message:
            "Связь между этими должностями в данном отделе уже существует",
        });
      }
  ...
  app.put("/api/positionpositions/:id", async (req: Request, res: Response) => {
    try {
      ...
      const updatedRelation = await storage.updatePositionPosition(
        id,
        updateData,
      );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(id)) {
          return res
            .status(400)
            .json({ status: "error", message: "Invalid relation ID" });
        }
        ...
        res.json({
          status: "success",
          message: "Position relation deleted successfully",
        });
      } catch (error) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        positionDepartments
          .filter(
            (link) => link.department_id === departmentId && !link.deleted,
          )
          .forEach((link) => {
        ...
        const departmentPositions = allPositions.filter(
          (position) =>
            positionIds.has(position.position_id) && !position.deleted,
        );
  ...
  app.put("/api/pd/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        "Updating position-department link:",
        id,
        "with data:",
        req.body,
      );
      ...
      const updatedLink = await storage.updatePositionDepartment(
        id,
        updateData,
      );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(id)) {
          return res
            .status(400)
            .json({ status: "error", message: "Invalid link ID" });
        }
        ...
        res.json({
          status: "success",
          message: "Position-department link deleted successfully",
        });
      } catch (error) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const positionEmployee = employees.find(
          (emp) => emp.position_id === position.position_id,
        );
        ...
        res
          ...
          .json({
            status: "error",
            message: "Failed to fetch position subordinates",
          });
      }
    },
  );
  ...
  app.get(
    "/api/positions/:id/with-departments",
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
        ...
        res
          ...
          .json({
            status: "error",
            message: "Failed to fetch position with departments",
          });
      }
    },
  );
    ...
    try {
      ...
      if (departmentId) {
        try {
          ...
          console.log(
            `Создана связь должности ID ${position.position_id} с отделом ID ${departmentId}`,
          );
        } catch (linkError) {
          console.error(
            "Ошибка при создании связи должность-отдел:",
            linkError,
          );
      ...
      if (departmentId) {
        try {
          ...
          if (!existingLink) {
            ...
            console.log(
              `Создана связь должности ID ${id} с отделом ID ${departmentId} при обновлении`,
            );
          }
        } catch (linkError) {
          console.error(
            "Ошибка при обновлении связи должность-отдел:",
            linkError,
          );
  ...
  app.get("/api/public-settings", async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: "error", message: "Failed to fetch public settings" });
    }
  });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!isCorrectPassword) {
          return res
            .status(400)
            .json({ status: "error", message: "Неверный текущий пароль" });
        }
        ...
        if (!updatedUser) {
          return res
            .status(500)
            .json({ status: "error", message: "Не удалось обновить пароль" });
        }
  ...
  app.post("/api/sort-tree", async (req: Request, res: Response) => {
    try {
      ...
      if (sortItemData.parent_id === null) {
        queryCondition = and(baseCondition, isNull(sort_tree.parent_id));
      } else {
        queryCondition = and(
          baseCondition,
      ...
      const existingSortItem = await db
        .select()
        .from(sort_tree)
  ...
  app.post("/api/sort-tree/reorder", async (req: Request, res: Response) => {
    try {
      ...
      if (
        !Array.isArray(items) ||
        items.some((item) => !item.id || typeof item.sort !== "number")
      ) {
        return res.status(400).json({
          status: "error",
