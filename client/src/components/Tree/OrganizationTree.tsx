...
import { positionsRelations } from "@shared/schema";
...
type OrganizationTreeProps = {
  ...
  handleGoBack: () => void;
  onRootViewChange: (value: boolean) => void;
  setShowVacancies: (value: boolean) => void;
...
const OrganizationTree: React.FC<OrganizationTreeProps> = (props) => {
        ...
        if (currentDepartmentId) {
          ...
          result.subordinates = result.subordinates.map((subNode: any) => {
            subNode.childDepartments = departments.filter(
              ...
              const employeesInDep = employees.find(
                (e) =>
          ...
          result.childDepartments = departments.filter(
            ...
            const positionsInDep = positionsWithDepartments.map((pos) => {
                const filtred = pos.departments.filter(
                  (d) =>
                    d.department_id === dep.department_id &&
                    d.parent_position === null
                );
                ...
                if (filtred.length > 0) {
                  return {
                    ...
                    department: dep,
                    employee: employees.find(employee => employee.position_id === pos.position_id && employee.department_id === dep.department_id),
                    childDepartments: [],
                    subordinates: []
                  };
                } else {
  ...
  const handlePositionClick = (
    ...
    const safePositionId = positionId ?? -1; // Для безопасности устанавливаем значение по умолчанию

    // Если positionId === 0 — возврат к корню
    if (safePositionId === 0) {
      setSelectedPositionId(0);
      setCurrentDepartmentContext(() => {
        window.currentDepartmentContext = null;
        return null;
      });
    ...
    setSelectedPositionId(safePositionId);

    if (onPositionClick) {
      const position = props.positionsData?.find(p => p.position_id === safePositionId);
  ...
  const handleGoBack = () => {
    if (navigationHistory.length > 0) {
      ...
      if (prevItem.departmentId) {
        setCurrentDepartmentContext(() => {
          window.currentDepartmentContext = prevItem.departmentId;
          return prevItem.departmentId;
        });
      }

      const positionId = prevItem.positionId;
      setSelectedPositionId(positionId || 0);
      ...
      onRootViewChange(true); // Устанавливаем флаг для отображения блоков по организациям
  ...
  useEffect(() => {
    ...
    if (hasAllData) {
      ...
      console.log(rootDepartmentHierarchy);
  ...
  useEffect(() => {
    ...
    if (
      ...
      setCurrentDepartmentContext(() => {
        window.currentDepartmentContext = currentDepartmentId;
      ...
      <div className="org-tree-container">
        ...
        >
          <div className="organization-controls">
            {selectedPositionId !== 0 && (
                <div className="position-navigation">
                  <button className="back-to-main-hierarchy" onClick={handleGoHome}>
                    ← В начало структуры
                  </button>
                </div>
            )}
          ...
          <PositionTree
              ...
              positionsWithDepartments={positionsWithDepartments}
