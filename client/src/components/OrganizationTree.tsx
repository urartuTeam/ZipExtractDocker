...
const OrganizationTree: React.FC<OrganizationTreeProps> = (props) => {
  ...
  const handlePositionClick = (
    ...
    console.log(
        `111 Клик по должности с ID: ${safePositionId}, контекст отдела: ${departmentContext || "не указан"}`
    );
  ...
  const handleGoBack = () => {
    if (navigationHistory.length > 0) {
      ...
      setSelectedPositionId(0);
      setCurrentDepartmentContext(null);
      setIsRootView(true); // Устанавливаем флаг для отображения блоков по организациям
