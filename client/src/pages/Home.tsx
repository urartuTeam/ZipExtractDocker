...
export default function Home() {
  ...
  if (positionsWithDepartments.length > 0) {
    ...
    console.log(
      "Данные positionsWithDepartmentsData инициализированы:",
  ...
  const totalPositionsFromDb = positionsWithDepartments.reduce(
    (total, position) => {
  ...
  const vacantPositionsCount = Math.max(
    0,
    totalPositionsCount - employeesCount,
  );
    ...
    <div className="flex flex-col">
      ...
      <div className="flex-1 p-4 bg-gray-100 flex flex-col">
        ...
        <div className="grid gap-4 md:grid-cols-4 flex-shrink-0">
          ...
          <Link
            href={`/vacancies${selectedPositionId ? "/" + selectedPositionId : ""}`}
          >
            <div className="bg-white p-4 rounded-md shadow-sm cursor-pointer hover:shadow-md transition-shadow">
              <div className="flex justify-between items-center mb-2">
              ...
              <div className="text-2xl font-bold">
                <span className="text-[#a40000]">
                  Всего: {totalPositionsCount}
                </span>{" "}
                <span className="text-green-600">({vacantPositionsCount})</span>
              </div>
              <div className="text-sm text-gray-500">
                Отчет по вакансиям организации
              </div>
            </div>
          </Link>
        </div>
