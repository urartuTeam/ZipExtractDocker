...
export default function Home() {
  ...
  const {
    data: positionPositionsResponse,
    isLoading: isLoadingPositionPositions,
  } = useQuery({
  ...
  const getContextProjectsCount = (departmentId: number | null): number => {
    ...
    const isOrganization =
      organizations.filter(
        (org) => org.department_id === selectedDepartment.department_id,
      ).length > 0;

    if (isOrganization) {
      return projects.filter(
        (p) => p.id_organization === selectedDepartment.department_id,
      ).length;
    } else if (selectedDepartment.parent_department_id) {
      return projects.filter(
        (p) => p.id_organization === selectedDepartment.parent_department_id,
      ).length;
    } else {
      const parentPosition = positionsWithDepartments.filter(
        (pd) => selectedDepartment.parent_position_id === pd.position_id,
      )[0];
      const organizationId = parentPosition?.departments.map(
        (d) => d.department_id,
      )[0];

      return projects.filter((p) => p.id_organization === organizationId)
        .length;
    }
  };
  ...
  const getAllChildDepartments = (context): number[] => {
    ...
    const result: number[] = [];
    const curDepartment = departments.filter(
    ...
    while (queue.length > 0) {
      ...
      const children = departments.filter(
        (d) => d.parent_department_id === currentId && !d.is_organization,
      );
      const children2 = departments.filter(
        (d) => d.parent_position_id === positionId,
      );
  ...
  const isLoading =
    ...
    isLoadingEmployees ||
    isLoadingProjects ||
    isLoadingPositionsWithDepartments ||
    isLoadingPositionPositions ||
    isLoadingOrganizations ||
    isLoadingSettings;

  const formatedPositionId =
    selectedPositionId % 1000 === 0
      ? selectedPositionId / 1000
      : selectedPositionId;
      ...
      <div className="flex-1 p-4 bg-gray-100 flex flex-col">
        ...
        <div className="rounded-md shadow-sm p-6 mb-8 flex-grow-0 h-full">
          {isLoading ? (
            ...
            <div
              className="w-full overflow-x-auto custom-scrollbar h-full"
