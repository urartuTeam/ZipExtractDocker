...
function calcPositionTotals(
    pos: Position & any,
    deptId: number,
    getPositionDepartmentInfo: any,
    getChildDeptsByPosition: any,
    getDeptPositions: any,
    getChildDeptsByDept: any
): { staffUnits: number; vacancies: number } {
  const { staffUnits, vacancies } = getPositionDepartmentInfo(pos.position_id, deptId);
  let totalStaff = staffUnits;
  let totalVac = vacancies;

  (pos.children_positions || []).forEach(child => {
    const c = calcPositionTotals(child, deptId, getPositionDepartmentInfo, getChildDeptsByPosition, getDeptPositions, getChildDeptsByDept);
    totalStaff += c.staffUnits;
    totalVac += c.vacancies;
  });

  const childDepts = getChildDeptsByPosition(pos.position_id);
  childDepts.forEach(d => {
    const deptTotals = calcDeptTotals(d.department_id, getPositionDepartmentInfo, getChildDeptsByPosition, getDeptPositions, getChildDeptsByDept);
    totalStaff += deptTotals.staffUnits;
    totalVac += deptTotals.vacancies;
  });

  return { staffUnits: totalStaff, vacancies: totalVac };
}

function calcDeptTotals(
    deptId: number,
    getPositionDepartmentInfo: any,
    getChildDeptsByPosition: any,
    getDeptPositions: any,
  ...
  getDeptPositions(deptId).forEach(pos => {
    const t = calcPositionTotals(pos, deptId, getPositionDepartmentInfo, getChildDeptsByPosition, getDeptPositions, getChildDeptsByDept);
    totalStaff += t.staffUnits;
    totalVac += t.vacancies;
  });

  getChildDeptsByDept(deptId).forEach(d => {
    const dt = calcDeptTotals(d.department_id, getPositionDepartmentInfo, getChildDeptsByPosition, getDeptPositions, getChildDeptsByDept);
    totalStaff += dt.staffUnits;
    totalVac += dt.vacancies;
  });
...
export default function Vacancies() {
  ...
  const [expEmpName, setExpEmpName] = useState<{ [k: string]: boolean }>({});
  ...
  const [, routeParamsWithType] = useRoute("/vacancies/:type/:id");
  ...
  const toggleEmpName = useCallback((key: string) => {
    setExpEmpName((prev) => ({ ...prev, [key]: !prev[key] }));
  }, []);
    ...
    positions.forEach((pos) => {
      if (isMatch(pos.name, searchTermLower)) {
        ...
        const findChildPositions = (parentId: number) => {
          const children_positions = positionRelations.filter(
            (rel) => !rel.deleted && rel.parent_position_id === parentId,
          );

          children_positions.forEach((child) => {
  ...
  const getDeptPositions = (deptId: number) => {
    ...
    linked.forEach((p) => (map[p.position_id] = { ...p, children_positions: [] }));
    ...
    rels.forEach((r) => {
      ...
      if (parent && child) {
        if (
          !parent.children_positions.some((c: any) => c.position_id === child.position_id)
        ) {
          parent.children_positions.push(child);
        }
      }
    });
  ...
  const renderPositionRow = (
    p:
      | (Position & { children_positions?: Array<Position & { children_positions?: any[] }> })
      | undefined,
    deptId: number,
    lvl = 0,
    parentId?: string,
    ...
    const rows = [];
    const fioEx = expEmpName[key] ?? false;

    const rowId = parentId ? `${parentId}-${key}` : key;
    const emps = getEmps(p.position_id, deptId);
    const childPositions = p.children_positions || [];
    const childDepts = getChildDeptsByPosition(p.position_id);
    let { staffUnits, vacancies } = getPositionDepartmentInfo(p.position_id, deptId);
    if (!ex && (childPositions.length > 0 || childDepts.length > 0)) {
      const agg = calcPositionTotals(
          p,
          deptId,
          getPositionDepartmentInfo,
          getChildDeptsByPosition,
          getDeptPositions,
          getChildDeptsByDept
      );
      staffUnits = agg.staffUnits;
      vacancies = agg.vacancies;
    }
          ...
          >
            {(childPositions.length > 0 || childDepts.length > 0) ? (
              (ex ? (
                ...
                emps.length !== 0 && (
                  fioEx ? (
                    <ChevronUp className="h-4 w-4 mr-1 text-neutral-500" />
                  ) : (
            ...
            <div>
              {emps.length === 1 ? (
                  <span className="flex items-center">{p.name} (<User className="h-4 w-4 mr-1 text-green-500" /> <span style={{ fontWeight: 400 }}>{emps[0]?.full_name}</span>)</span>
              ) : (
                <>
                  <div className="flex items-center" onClick={() => toggleEmpName(key)}>
                    {p.name}
                  </div>
                  {fioEx && (
                    <ul className="pl-0 mt-2 translate-x-[-25px]">
                      {emps.map(emp => (
                          <li className="flex items-center mb-1"><User className="h-4 w-4 mr-1 text-green-500"/> <span
                              style={{fontWeight: 400}}>{emp?.full_name}</span></li>
                      ))}
                    </ul>
                  )}
                </>
    ...
    if (ex) {
      childPositions.forEach((child) => {
        // if (child && !child.department_id) {
        //   // Проверяем, что дочерний элемент существует
        //   rows.push(...renderPositionRow(child, deptId, lvl + 1, rowId));
        // } else if (child.department_id === deptId) {
        //   rows.push(...renderPositionRow(child, deptId, lvl + 1, rowId));
        // }

        if (child) {
          if (!child.department_id || child.department_id === deptId) {
            // Проверяем, что дочерний элемент существует и либо не имеет department_id,
            // либо его department_id совпадает с deptId
            rows.push(...renderPositionRow(child, deptId, lvl + 1, rowId));
          }
        }
      });
      ...
      childDepts.forEach((dept) => {
        if (dept) {
          // Проверяем, что дочерний отдел существует
  ...
  const renderDepartmentRow = (d: Department, lvl = 0, parentId?: string) => {
    ...
    // Рассчитываем суммарные значения для отдела
    let { staffUnits, vacancies } = calcDeptTotals(
        d.department_id,
        getPositionDepartmentInfo,
        getChildDeptsByPosition,
        getDeptPositions,
    ...
    // Если поддерево не развернуто и есть дочерние элементы, показываем агрегированные данные
    if (!ex && (deptPositions.length > 0 || childDepts.length > 0)) {
      const agg = calcDeptTotals(
          d.department_id,
          getPositionDepartmentInfo,
          getChildDeptsByPosition,
          getDeptPositions,
    ...
    rows.push(
        <TableRow key={rowId}>
          ...
          <TableCell className="text-center">{staffUnits}</TableCell>
          <TableCell className="text-center">{staffUnits - vacancies}</TableCell>
          <TableCell className="text-center">
            {vacancies > 0 ? (
                <span className="text-green-600 font-medium">{vacancies}</span>
            ) : (
  ...
  const renderFromRoute = () => {
    ...
    if (department.length > 0 && routeParamsWithType?.type === 'department') {
      return renderDepartmentRow(department[0], 0);
    } else if (position.length > 0) {
      return renderPositionRow(position[0], window.currentDepartmentContext ?? 0);
    }
    return [];
  };
  ...
  let targetDepartment = routeParamsWithType?.type === 'department'
    ? departments.find((d) => !d.deleted && d.department_id === +routeParamsWithType?.id)
    : departments.find(d => (
      d.department_id === positions.filter(p => p.position_id === Number(routeParams?.id))[0].departments[0].department_id
    ))
        ...
        <Card>
          <CardHeader className="flex flex-col space-y-4">
            ...
            <div>
              <CardTitle>
                {/* {showOnlyTarget */}
                  {/* ? `Учет вакансий | ${targetDepartment?.name || stateOrgName}` */}
                Учет вакансий
              </CardTitle>
              <CardDescription>
                ...
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead colSpan={3}>
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <Tooltip>
                        <TooltipTrigger>
                          <Button className="p-0 h-auto size-4 rounded-[50%]">
                            i
                          </Button>
                        </TooltipTrigger>
                        <TooltipContent className="text-center">
                          Нажатием на <br></br>"Занято" или "Вакантно" <br></br>
                          можно отфильтровать структуру <br></br>организации,
                          оставив выбранное.
                        </TooltipContent>
                      </Tooltip>
                    </div>
